...
const Tornado = artifacts.require('./BEP20Bermuda.sol')
const BadRecipient = artifacts.require('./BadRecipient.sol')
const Token = artifacts.require('./BEP20Mock.sol')
const { BNB_AMOUNT, TOKEN_AMOUNT, MERKLE_TREE_HEIGHT, BEP20_TOKEN } = process.env
...
contract('BEP20Bermuda', accounts => {
  let tornado
  let token
  let usdtToken
  let badRecipient
  ...
  const refund = BNB_AMOUNT || '1000000000000000000' // 1 ether
  ...
  describe('#deposit', () => {
    ...
    it('should not allow to send ether on deposit', async () => {
      ...
      error.reason.should.be.equal('BNB value is supposed to be 0 for BEP20 instance')
    })
  })
