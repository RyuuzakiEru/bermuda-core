...
const Bermuda = artifacts.require('./BNBBermuda.sol')
...
contract('BNBBermuda', accounts => {
  let tornado
  const sender = accounts[0]
  const operator = accounts[0]
  const levels = MERKLE_TREE_HEIGHT || 16
  const value = BNB_AMOUNT || '1000000000000000000' // 1 ether
  let snapshotId
  let prefix = 'test'
  let tree
  ...
  before(async () => {
    ...
    tornado = await Bermuda.deployed()
  ...
  describe('#withdraw', () => {
    ...
    it('should reject with non zero refund', async () => {
      ...
      const error = await tornado.withdraw(proof, ...args, { from: relayer }).should.be.rejected
      error.reason.should.be.equal('Refund value is supposed to be zero for BNB instance')
    })
  })
